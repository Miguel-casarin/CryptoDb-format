...
db_file = r'C:\Users\migue\Documents\cryptodb2\format\crypto4.db'
...
def processar_csv_e_inserir():
    ...
    with open(csv_file, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            # Verificar se a moeda já existe na tabela ds_coin
            cursor.execute("SELECT coin_id FROM coin WHERE coin_name = ?", (coin,))
            result = cursor.fetchone()
            if result is None:
                cursor.execute("INSERT INTO coin (coin_name) VALUES (?)", (coin,))
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO price (coin_id, date_hour, price, volume, market_camp)
                VALUES (?, ?, ?, ?, ?)
    ...
    # Processar e inserir na tabela movement_diary
    for (coin_id, date), prices in day_coins.items():
        ...
        cursor.execute("""
            INSERT OR REPLACE INTO movement_diary (date, coin_id, open, close, max, lower)
            VALUES (?, ?, ?, ?, ?, ?)
